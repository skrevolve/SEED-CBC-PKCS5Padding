...
<html>
<head>
	<title>WPAY 표준 결제정보(오픈뱅킹 계좌) 등록 결과</title>
...
<%
	...
	String param_payMethod 		= paramMap.get("payMethod");		// 이니시스에서 발행한 payMethod  	- (SEED 암호화 대상필드)
	String param_bankCardCode	= paramMap.get("bankCardCode");		// 이니시스에서 발행한 bankCardCode  - (SEED 암호화 대상필드)
	String param_bankCardNo 	= paramMap.get("bankCardNo");		// 이니시스에서 발행한 bankCardNo 	- (SEED 암호화 대상필드)
	String param_cardName 		= paramMap.get("cardName");			// 이니시스에서 발행한 cardName 	 	- (SEED 암호화 대상필드)
	String param_wpayTokenNcnm 	= paramMap.get("wpayTokenNcnm");	// 이니시스에서 발행한 wpayTokenNcnm - (SEED 암호화 대상필드)
	...
	out.println("<br/>");
	out.println("#### WPAY 표준 결제정보(오픈뱅킹 계좌) 등록 결과 signature 확인 실패 ####");
	out.println("<pre>");
	...
	System.out.println(e);
		...
		*/
		...
		out.println("<br/>");
		out.println("#### WPAY 표준 결제정보(오픈뱅킹 계좌) 등록 실패 ####");
		out.println("<pre>");
		...
		<tr>
			<td bgcolor="6095BC" align="center" style="padding:10px">
				<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF" style="padding:20px">
					...
					<tr>
						<td >
							<span style="font-size:20px"><b>WPAY 표준 결제정보(오픈뱅킹 계좌) 등록 결과 파라미터 정보</b></span><br/>
						</td>
					</tr>
					<tr >
						<td >
